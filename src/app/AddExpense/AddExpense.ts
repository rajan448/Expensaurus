...
export class AddExpense implements OnInit {
  ...
  async submitExpense() {
    try {
      ...
      this.getExpensesSrvc.addExpense(this.expense).subscribe((res) => {
  ...
  async formPayload() {
    ...
    this.expense.date = new Date(this.date).getTime().toString();
    ...
    if (this.currency !== Currency.INR) {
      ...
      this.expense.rupeeEquivalent = Math.ceil(<number>this.amount * 75);
